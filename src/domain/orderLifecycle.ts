 ...
 * Implements Recommendation #3 (Richer Lifecycle & Guarded Transitions)
 */

import { OrderStatus, OrderStatusHistoryEntry, Order } from '../types';
 ...
 * Informs whether immediate cancellation is allowed or requires administrative exception review.
 */
  ...
  switch (orderStatus) {
    case 'PENDING':
      return {
        ...
        explanation: 'Annulation immédiate autorisée sans frais (le commerce n\'a pas encore commencé la préparation).',
      };
    case 'CONFIRMED':
      if (requesterRole === 'CUSTOMER') {
        return {
          ...
          requiresStoreApproval: false,
          requiresAdminIntervention: false,
          explanation: 'Annulation client acceptée avant le lancement de la cuisine.',
        };
      }
      return {
        allowedImmediately: true,
        requiresStoreApproval: false,
        requiresAdminIntervention: false,
        explanation: 'Annulation autorisée par le commerçant ou le système.',
      };
    case 'PREPARING':
      return {
