    ...
    <div className="fixed inset-0 z-50 bg-black/65 backdrop-blur-xs flex items-end justify-center">
      <div className="w-full max-w-[430px] bg-white rounded-t-3xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom duration-200">
        ...
        <div className="p-3 pb-2 border-b border-neutral-100 bg-white sticky top-0 z-20">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1.5">
              <span className="bg-[#1C1B1B] text-white text-[10px] font-bold px-2 py-0.5 rounded flex items-center gap-1">
                <ChefHat size={11} className="text-[#D9943B]" />
                {storeName}
              </span>
            ...
            <button
              onClick={onClose}
              aria-label="Fermer"
              className="w-7 h-7 rounded-full bg-neutral-100 flex items-center justify-center text-zinc-600 hover:bg-neutral-200 active:scale-95"
            >
              <X size={16} />
            </button>
          </div>
          ...
          <div className="mt-2.5 flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="font-bold text-[15px] text-[#1C1B1B] leading-tight line-clamp-1">{dish.name}</h2>
              <p className="text-xs text-zinc-500 line-clamp-1 mt-0.5">{dish.description}</p>
              <div className="flex items-center gap-2 mt-1">
                <div className="flex items-baseline gap-1">
                  <span className="text-[#FF5722] font-bold text-xs">DZD</span>
                  <span className="text-[#FF5722] font-extrabold text-[18px] leading-none">{dish.price}</span>
                  {dish.originalPrice && (
                    <span className="text-zinc-400 text-xs line-through ml-1">{dish.originalPrice} DZD</span>
                  )}
                </div>
                <span className="bg-orange-50 text-[#FF5722] border border-orange-200/50 text-[10px] px-1.5 py-0.5 rounded font-bold">
                  🌶️ Harissa Douce
                </span>
              </div>
            </div>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-3.5 space-y-3.5">
          ...
          <section className="bg-white rounded-xl p-3 border border-neutral-100 shadow-xs">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                <h3 className="font-bold text-[14px] text-[#1C1B1B]">{portionGroup.name}</h3>
                <span className="bg-amber-100 text-amber-900 text-[10px] font-bold px-1.5 py-0.2 rounded">Requis</span>
              </div>
              <span className="text-[11px] text-zinc-400 font-medium">Choix unique</span>
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-[13px] font-semibold text-[#1C1B1B]">{opt.name}</span>
                    </div>
          ...
          <section className="bg-white rounded-xl p-3 border border-neutral-100 shadow-xs">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                <h3 className="font-bold text-[14px] text-[#1C1B1B]">{spicyGroup.name}</h3>
                <span className="bg-amber-100 text-amber-900 text-[10px] font-bold px-1.5 py-0.2 rounded">Requis</span>
              </div>
              <span className="text-[11px] text-zinc-400 font-medium">Choix unique</span>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`p-2.5 rounded-xl border flex flex-col items-center justify-center text-center relative transition-all ${
                      isSelected
                        ? 'border-[#D9943B] bg-amber-50/50 ring-1 ring-[#D9943B]'
                        : 'border-neutral-100 hover:border-neutral-200 bg-neutral-50/40'
                    }`}
                  >
                    {isSelected && (
                    ...
                    <span className="text-[12px] font-bold text-[#1C1B1B]">{opt.name}</span>
                    <span className="text-[10px] text-[#FF5722] font-semibold mt-0.5">
          ...
          <section className="bg-white rounded-xl p-3 border border-neutral-100 shadow-xs">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                <h3 className="font-bold text-[14px] text-[#1C1B1B]">{sidesGroup.name}</h3>
                <span className="bg-amber-100 text-amber-900 text-[10px] font-bold px-1.5 py-0.2 rounded">Requis</span>
              </div>
              <span className="text-[11px] text-zinc-400 font-medium">1 au choix</span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {sidesGroup.options.map((opt) => {
                ...
                return (
                  ...
                  >
                    <span className="text-[12px] font-semibold text-[#1C1B1B]">{opt.name}</span>
          ...
          <section className="bg-white rounded-xl p-3 border border-neutral-100 shadow-xs">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                <h3 className="font-bold text-[14px] text-[#1C1B1B]">{addonsGroup.name}</h3>
                <span className="bg-neutral-100 text-zinc-600 text-[10px] font-semibold px-1.5 py-0.2 rounded">Optionnel</span>
              </div>
              <span className="text-[11px] text-zinc-400 font-medium">Sélection multiple</span>
            </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-4 h-4 rounded flex items-center justify-center ${
                          isChecked ? 'bg-[#D9943B] text-black font-bold' : 'border border-zinc-300'
                        }`}
                      >
                        {isChecked && <Check size={11} strokeWidth={3} />}
                      </div>
                      <span className="text-[12px] text-[#1C1B1B] font-medium">{opt.name}</span>
                    </div>
                    <span className="text-xs font-bold text-[#FF5722]">+{opt.priceDelta} DZD</span>
                  </label>
                );
              })}
            </div>
          ...
          <section className="bg-white rounded-xl p-3 border border-neutral-100 shadow-xs">
            <div className="flex items-center justify-between mb-2">
              <h3 className="font-bold text-[14px] text-[#1C1B1B]">Préférences de Préparation</h3>
              <span className="text-[11px] text-zinc-400 font-medium">Touchez pour activer</span>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-semibold flex items-center gap-1 transition-all ${
                      isActive
                        ? 'bg-amber-100 text-amber-900 border border-[#D9943B] shadow-xs'
                        : 'bg-neutral-100 text-zinc-600 border border-transparent hover:bg-neutral-200'
                    }`}
                  >
                    {isActive && <Check size={12} className="text-[#B02F00]" />}
                    <span>{pref}</span>
                  </button>
                );
              })}
          ...
          <section className="bg-white rounded-xl p-3 border border-neutral-100 shadow-xs">
            <div className="flex items-center justify-between mb-1.5">
              <label htmlFor="chef-remark" className="font-bold text-[13px] text-[#1C1B1B] flex items-center gap-1">
                <Sparkles size={14} className="text-[#D9943B]" />
                Instructions spéciales pour le cuisinier
              </label>
              <span className="text-[10px] text-zinc-400">{chefRemark.length}/50</span>
            </div>
            <textarea
              id="chef-remark"
              rows={2}
              ...
              className="w-full text-xs bg-neutral-50 border border-neutral-200 rounded-lg p-2 focus:outline-none focus:border-[#D9943B] resize-none"
            />
          </section>
        </div>

        {/* Sticky Action Footer */}
        <footer className="p-3 bg-white border-t border-neutral-100 shadow-lg">
          <div className="flex items-center justify-between text-[11px] text-zinc-500 mb-2 px-1">
            <div className="truncate max-w-[280px]">
              <span className="font-semibold text-zinc-800">Sélection:</span> Standard · Harissa Maison · Frites
            </div>
            <span className="text-[#00B578] font-bold text-[11px] flex items-center gap-0.5">
              ⚡ Préparation rapide
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            {/* Stepper */}
            <div className="flex items-center bg-neutral-100 rounded-full p-1 border border-neutral-200 gap-2.5">
              <button
                onClick={() => setQuantity(Math.max(1, quantity - 1))}
                aria-label="Diminuer quantité"
                className="w-7 h-7 rounded-full bg-white border border-neutral-300 flex items-center justify-center text-zinc-700 active:scale-95"
              >
                <Minus size={14} />
              </button>
              <span className="font-bold text-sm text-[#1C1B1B] min-w-[16px] text-center">{quantity}</span>
              <button
                onClick={() => setQuantity(quantity + 1)}
                aria-label="Augmenter quantité"
                className="w-7 h-7 rounded-full bg-[#D9943B] text-[#1C1B1B] flex items-center justify-center active:scale-95 shadow-xs"
              >
                <Plus size={14} />
              </button>
            </div>
            ...
            <button
              ...
              className="flex-1 h-11 bg-[#D9943B] hover:brightness-105 active:scale-[0.98] text-[#1C1B1B] rounded-full px-4 flex items-center justify-between shadow-md transition-transform font-bold"
            >
              <div className="flex items-baseline gap-1 text-left">
                <span className="text-[10px] uppercase font-semibold text-zinc-800 opacity-80">Total</span>
                <span className="text-base font-extrabold">{totalPrice} DZD</span>
              </div>
